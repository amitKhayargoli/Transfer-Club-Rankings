...
async def run_analytics(session: AsyncSession) -> dict:
    """Re-run the full analytics pipeline (buy-sell pairs + club metrics + windows).

    This calls the existing services from api.services.analytics.

    Returns:
        dict with keys: pairs_computed, clubs_updated, windows_updated
    """
    from api.services.analytics import compute_buy_sell_pairs, compute_club_metrics, compute_all_window_metrics
    ...
    logger.info("Updated window metrics: %s", window_counts)
